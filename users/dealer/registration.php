...
<?php
    ...
    if (isset($_REQUEST['username'])) {
      ...
      //  $user_id = 123;//get highest user_id form users table add 1 to it.we use max function
        $query    = "INSERT into `dealerlogin` ( username,first_name, last_name,usertype, email, password,
          create_datetime,mobile_number, address, city, state)
                     VALUES ('$user_name', '$first_name', '$last_name','Dealer',
                      '$email','$password','$create_datetime','$mobile_number', '$address',
                     '$city','$state')";
...
<br>
        ...
        <input type="text" class="login-input" name="state" placeholder="State">
